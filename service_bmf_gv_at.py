...
class Handler(Extract, GetPages):
    ...
    def get_working_tree_api(self, link_name, type, method='GET', data=None):
        ...
        if type == 'api':
            ...
            #print(self.api.content[18:])
            #self.api = json.loads(self.api.content)
            self.api = text.split(']')
            #myList = [i.repla]
            # print(re.findall("\['Lesandro GmbH.*\\\\r.*\[", str(self.api.content)))
    ...
    def get_overview(self, link_name):
        ...
        try:
            ...
            self.overview['vcard:organization-name'] = re.findall("'.*'", temp)[0].replace("'",'').strip()
        except:
            return None

        self.overview['isDomiciledIn'] = 'AT'
        self.overview['bst:sourceLinks'] = ['https://service.bmf.gv.at/service/allg/lsu/']
